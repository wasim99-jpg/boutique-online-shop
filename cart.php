    ...
    if($value['code'] === $_POST["code"]){
	 ...
	 if(isset($_POST['checkout'])) { 
            if(!isset($_SESSION["loggedin"]) ){
				echo"<script> alert('Please login before proceeding to checkout');";
			echo"window.location.href='login.php'  </script>";}
			else{
				
			header("location: paymentTest.php");
				exit;
...
<br>
   ...
   <form method="post"> 
        <input type="submit" name="checkout" class="checkout"
                value="Checkout"/> 
       
    </form> 
  
	
	<a style="position: absolute; right: 612px;"class="shop" href="shop.html" >continue shopping</a>
	</div>
  <?php
}else{
